...
abstract class PaginatedResponse[A] {
  def data: List[A]
  def pagination: Option[TwitchPagination]
  def pageData: List[A] = data
  def pageCursor: Option[String] = pagination.flatMap(_.cursor)
}
